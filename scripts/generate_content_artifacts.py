...
def sort_source_files(items: list[SourceFile]) -> list[SourceFile]:
    return sorted(items, key=lambda sf: (sf.logical_id, sf.branch, sf.path, sf.sha))
...
def main() -> int:
    ...
    parser.add_argument("--strict-branches", action="store_true", help="Exit non-zero when preferred branches are missing locally.")
    ...
    if missing_preferred:
        ...
        if args.strict_branches:
            raise SystemExit("Missing preferred branches in local clone")
    ...
    selected_sources = sort_source_files(list(seen_logical.values()))
    ...
    manifest = {
        ...
        "source_files": [asdict(sf) for sf in sort_source_files(source_files)],
